...
mod tests {
    ...
    fn test_get_sneaky_numbers_1() {
        ...
        assert_eq!([0, 1].to_vec(), Solution::get_sneaky_numbers(nums));
    }

    #[test]
    fn test_get_sneaky_numbers_2() {
        let nums = [0, 3, 2, 1, 3, 2].to_vec();
        assert_eq!([2, 3].to_vec(), Solution::get_sneaky_numbers(nums));
    }

    #[test]
    fn test_get_sneaky_numbers_31() {
        let nums = [7, 1, 5, 4, 3, 4, 6, 0, 9, 5, 8, 2].to_vec();
        assert_eq!([4, 5].to_vec(), Solution::get_sneaky_numbers(nums));
    }
}
